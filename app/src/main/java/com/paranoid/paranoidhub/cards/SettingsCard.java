...
public class SettingsCard extends Card {
    ...
    public SettingsCard(Context context, AttributeSet attrs, Bundle savedInstanceState) {
        ...
        seekBar.setThumbColor(getResources().getColor(R.color.red_400), getResources().getColor(R.color.red_900));
        seekBar.setScrubberColor(getResources().getColor(R.color.red_900));
        seekBar.setOnProgressChangeListener(new DiscreteSeekBar.OnProgressChangeListener() {
            @Override
            public void onProgressChanged(DiscreteSeekBar seekBar, int value, boolean fromUser) {
                SettingsHelper.savePreference(SettingsHelper.PROPERTY_CHECK_TIME, value * 3600 * 1000); //in ms
            }
