...
public abstract class Updater implements Response.Listener<JSONObject>, Response.ErrorListener {
    ...
    public void onResponse(JSONObject response) {
        ...
        try {
            ...
            if (lastUpdates.length > 0) {
                mServerWorks = true;
                if (mFromAlarm) {
                    OTAUtils.showNotification(getContext(), lastUpdates);
                }
            } else {
    ...
    private boolean versionError(String error) {
        ...
        if (!mFromAlarm && !mServerWorks) {
            ...
            if (error != null) {
                OTAUtils.showToastOnUiThread(getContext(), getContext().getResources().getString(id)
                        + ": " + error);
            } else {
                OTAUtils.showToastOnUiThread(getContext(), id);
            }
        }
        mCurrentServer = -1;
